...
PING_TARGET = "1.1.1.1"
WIRELESS_INTERFACE = "wlan0"  # Primary wireless interface
LAN_INTERFACE = "eth0"        # Primary LAN interface, set to None if not used or to be ignored
BUTTON_PIN_1 = 23
BUTTON_PIN_2 = 24

LOOP_SLEEP_INTERVAL = 0.1
PING_SCAN_IP_INTERVAL = 30   # How often to run ping, scan, wireless metrics, IP check (seconds)
SPEEDTEST_CHECK_INTERVAL = 60 # How often to start/check speedtest (seconds)

# Files for persistent data and configuration
IDENTIFIER_FILE = "/var/local/network_monitor_identifier.txt"
REGISTRAR_CONFIG_FILE = "/var/local/network_monitor_registrar_url.txt" # e.g. https://server:5001/register
API_KEY_FILE = "/var/local/network_monitor_api_key.txt" # Shared API key
...
API_REPORT_INTERVAL = 300 # Report to registrar every 5 minutes (heartbeat)
...
current_ip_labels = {} # Stores {interface_name: ip_address}
last_api_report_time = 0
last_reported_ip_for_api = None # Tracks the IP last successfully reported to the registrar
...
def get_ip_address_for_interface(interface_name):
    ...
    try:
        ...
        print(f"  Could not get IP for {interface_name}: {e}") # Less verbose for a helper
    return None
...
def run_ping_checks(target=PING_TARGET, interface_to_use=None):
    ...
    try:
        ...
        if times:
            ...
            if ttl_matches: # TTL might not be in all ping replies (e.g., first can be from gateway)
        ...
        if len(times) >= 2:
            ...
            jitter = 0 # Jitter is 0 if only one reply
...
def update_wireless_metrics(interface=WIRELESS_INTERFACE):
    signal_level = -100 # Default to very low signal if not found
    quality_percentage = -1
    if not interface:
        return # No wireless interface configured
    ...
    try:
        ...
        elif "Signal level" not in output: # Interface might not be wireless or not connected
...
def run_speedtest_child(result_queue, interface_to_use_for_source_ip=None):
    ...
    try:
        ...
        st_args = {"secure": True} # HTTPS for speedtest.net communication
        if source_ip:
        ...
        result_data['ping'] = results_dict.get('ping', -1) # Ping from speedtest server
    ...
    except speedtest.SpeedtestException as e: # Catch other speedtest-specific errors
...
def parse_nmcli_wifi_line(line, header_map):
    ap = {}
    parts = re.split(r'\s{2,}', line.strip()) # Split on 2 or more spaces
    try:
        bssid_idx = header_map.get("BSSID")
        ssid_idx = header_map.get("SSID")
        chan_idx = header_map.get("CHAN")
        signal_quality_idx = header_map.get("SIGNAL") # nmcli SIGNAL is 0-100 quality

        if bssid_idx is not None and bssid_idx < len(parts): ap['bssid'] = parts[bssid_idx]
        if ssid_idx is not None and ssid_idx < len(parts): ap['ssid'] = parts[ssid_idx]
        if chan_idx is not None and chan_idx < len(parts): ap['channel'] = parts[chan_idx]

        if signal_quality_idx is not None and signal_quality_idx < len(parts):
            quality_str = parts[signal_quality_idx]
            if quality_str and quality_str != '--':
                quality = int(quality_str)
                if 0 <= quality <= 100:
                    # Approximate conversion from quality (0-100) to dBm
                    # This is a rough heuristic: dBm = (Quality / 2) - 100
             ...
             ap['signal_dbm'] = -103 # Signal column not found or parse error


        if ap.get('ssid') == '--': ap['ssid'] = '<hidden_or_empty>'

    except (IndexError, ValueError) as e:
        # print(f"  Error parsing nmcli line part: '{line}'. Error: {e}") # Can be noisy
        return None
    if all(k in ap for k in ('ssid', 'bssid', 'channel', 'signal_dbm')):
        return ap
    return None

def get_nmcli_header_map(header_line):
    headers_raw = re.split(r'\s{2,}', header_line.strip())
    header_map = {name.upper().replace('*','').strip(): i for i, name in enumerate(headers_raw)} # Remove IN-USE '*'
    required_headers = ["BSSID", "SSID", "CHAN", "SIGNAL"]
    if not all(h in header_map for h in required_headers):
        print(f"  Error: nmcli output missing one of required headers: {required_headers}. Found: {list(header_map.keys())}")
        return None
    return header_map
...
def scan_wifi_aps_nmcli(interface=WIRELESS_INTERFACE):
    ...
    try:
        ...
        print(f"  Rescan command sent. Waiting a moment...")
        time.sleep(3)
    except subprocess.TimeoutExpired: print(f"  nmcli rescan command timed out for {interface}.")
    except FileNotFoundError: print("  Error: 'nmcli' command not found for rescan."); return
    except Exception as e: print(f"  Unexpected error during nmcli rescan: {e}")
    ...
    try:
        ...
        if not output_lines: print("  nmcli output is empty after rescan."); return

        header_line_index = 0
        if output_lines[0].lstrip().startswith("IN-USE"): # More robust check for header
             header_line_index = 0 # Header is the first line
        elif output_lines[0].lstrip().startswith("*"): # Old nmcli IN-USE marker
             header_line_index = 1

        if header_line_index >= len(output_lines): print("  nmcli output too short for header."); return

        header_map = get_nmcli_header_map(output_lines[header_line_index])
        if not header_map: return

        for line_num, line_content in enumerate(output_lines[header_line_index + 1:]):
            clean_line = line_content.lstrip().replace('*','',1).lstrip() # Remove IN-USE marker if present
            ap_data = parse_nmcli_wifi_line(clean_line, header_map)
            if ap_data:
                aps.append(ap_data)
    except subprocess.TimeoutExpired: print(f"  nmcli list command timed out for {interface}")
    except subprocess.CalledProcessError as e: print(f"  Failed to run nmcli list on {interface}: {e}. Output: {e.stderr.strip()}")
    except FileNotFoundError: print("  Error: 'nmcli' command not found for listing."); return
    ...
    valid_aps_count = 0
    for ap in aps:
        if ap['bssid'] not in reported_bssids:
            try:
                sanitized_ssid = re.sub(r'[^a-zA-Z0-9_:]', '_', ap['ssid'])
                if not sanitized_ssid: sanitized_ssid = "_empty_or_hidden_"
                WIFI_AP_SIGNAL.labels(
                    ssid=sanitized_ssid, bssid=ap['bssid'], channel=str(ap['channel'])
                ).set(ap['signal_dbm'])
    ...
    if not aps: print("  No APs found in nmcli scan after rescan.")

def update_device_ip(interface_to_check):
    global current_ip_labels
    if not interface_to_check: return # Skip if interface name is None or empty
    ...
    except FileNotFoundError: print(f"  Error: 'ip' command not found (checking {interface_to_check}).")
    except subprocess.TimeoutExpired: print(f"  'ip addr show' command timed out for {interface_to_check}")
    except subprocess.CalledProcessError: print(f"  Failed to get IP for {interface_to_check} (interface might be down/not exist).")
    except Exception as e: print(f"  Error getting IP address for {interface_to_check}: {e}")

    last_known_ip = current_ip_labels.get(interface_to_check)
    if current_ip_for_this_interface != last_known_ip:
        if last_known_ip:
            try: NETWORK_INTERFACE_INFO.remove(interface_to_check, last_known_ip); print(f"  Removed old IP label: {interface_to_check} / {last_known_ip}")
            except KeyError: pass
            except Exception as e: print(f"  Error removing old IP label {interface_to_check} / {last_known_ip}: {e}")
        if current_ip_for_this_interface:
            try:
                ...
                if interface_to_check in current_ip_labels: del current_ip_labels[interface_to_check]
        else:
             if interface_to_check in current_ip_labels: del current_ip_labels[interface_to_check]

# --- Identifier Functions --- (Same as previous version)
        ...
        with open('/proc/cpuinfo', 'r') as f:
            for line in f:
                if line.startswith('Serial'):
                    ...
                    if serial_match: serial = serial_match.group(1); break
    except Exception as e: print(f"  Warning: Could not read serial from /proc/cpuinfo: {e}")
...
def save_identifier(identifier):
    try:
        ...
        print(f"  Identifier saved to {IDENTIFIER_FILE}"); return True
    except Exception as e: print(f"  ERROR: Could not write identifier file {IDENTIFIER_FILE}: {e}"); return False

def load_identifier():
    if os.path.exists(IDENTIFIER_FILE):
        try:
            with open(IDENTIFIER_FILE, 'r') as f: identifier = f.read().strip()
            if identifier: print(f"  Loaded identifier from {IDENTIFIER_FILE}: {identifier}"); return identifier
        except Exception as e: print(f"  Error reading identifier file {IDENTIFIER_FILE}: {e}")
    print(f"  Identifier file {IDENTIFIER_FILE} not found or empty."); return None
...
def update_prometheus_identifier(new_identifier):
    ...
    except Exception as e: print(f"  ERROR setting new Prometheus identifier label '{new_identifier}': {e}"); return
...
def handle_identifier_update():
    ...
    if save_identifier(new_id): update_prometheus_identifier(new_id)
    else: print("--- Identifier Update Failed (Could not save) ---")
    print("--- Identifier Update Complete ---\n")

# --- GPIO Setup and Button Check --- (Same as previous version)
def setup_gpio():
    try:
        GPIO.setwarnings(False); GPIO.setmode(GPIO.BCM)
        GPIO.setup(BUTTON_PIN_1, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(BUTTON_PIN_2, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        print(f"GPIO pins {BUTTON_PIN_1} and {BUTTON_PIN_2} setup complete."); return True
    except Exception as e: print(f"ERROR setting up GPIO: {e}"); return False

def check_buttons():
    global buttons_currently_pressed
    try:
        b1_state = GPIO.input(BUTTON_PIN_1); b2_state = GPIO.input(BUTTON_PIN_2)
        if b1_state == GPIO.LOW and b2_state == GPIO.LOW:
            if not buttons_currently_pressed:
                ...
                handle_identifier_update(); buttons_currently_pressed = True
        elif b1_state == GPIO.HIGH and b2_state == GPIO.HIGH:
            if buttons_currently_pressed: print("Buttons released."); buttons_currently_pressed = False
    except Exception as e: print(f"An unexpected error occurred during button check: {e}")

# --- Configuration Loading Functions --- (Same as previous version)
def load_pi_api_key():
    global PI_SHARED_API_KEY; key = None
    if os.path.exists(API_KEY_FILE):
        try:
            with open(API_KEY_FILE, 'r') as f: key = f.read().strip()
            if key: print(f"  Successfully loaded API key from {API_KEY_FILE}."); PI_SHARED_API_KEY = key
            else: print(f"  Warning: API key file {API_KEY_FILE} is empty.")
        except Exception as e: print(f"  Warning: Error reading API key file {API_KEY_FILE}: {e}")
    else: print(f"  Warning: API key file {API_KEY_FILE} not found.")
...
def load_and_set_registrar_url():
    global REGISTRAR_API_URL; loaded_from_file = False
    if os.path.exists(REGISTRAR_CONFIG_FILE):
        try:
            with open(REGISTRAR_CONFIG_FILE, 'r') as f: url_from_file = f.read().strip()
            if url_from_file:
                if not url_from_file.startswith("https://"): print(f"  Warning: URL from {REGISTRAR_CONFIG_FILE} ('{url_from_file}') isn't HTTPS.")
                REGISTRAR_API_URL = url_from_file; loaded_from_file = True
            else: print(f"  Info: Registrar URL file '{REGISTRAR_CONFIG_FILE}' is empty.")
        except Exception as e: print(f"  Warning: Error reading registrar URL file '{REGISTRAR_CONFIG_FILE}': {e}")
    else: print(f"  Info: Registrar URL configuration file '{REGISTRAR_CONFIG_FILE}' not found.")
    if loaded_from_file: print(f"  Registrar API URL configured from file: {REGISTRAR_API_URL}")
    else:
...
# --- HTTP SD API Reporting Function --- (Same as previous version)
def report_ip_to_api(identifier, ip_address, port):
    global last_reported_ip_for_api, PI_SHARED_API_KEY, REGISTRAR_API_URL
    if not identifier or not ip_address: print("  [API Report] Skipping: Missing identifier or IP."); return False
    if not PI_SHARED_API_KEY: print("  [API Report] Skipping: Pi's API key not loaded."); return False
    if "PLEASE_CONFIGURE_IN_FILE" in REGISTRAR_API_URL: print(f"  [API Report] Skipping: REGISTRAR_API_URL not configured."); return False
    if not REGISTRAR_API_URL.startswith("https://"): print(f"  [API Report] Warning: REGISTRAR_API_URL ('{REGISTRAR_API_URL}') not HTTPS.")
    ...
    if not os.path.exists(verify_path):
        ...
        verify_path = False; urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    try:
        ...
        last_reported_ip_for_api = ip_address; return True
    except requests.exceptions.SSLError as e: print(f"  [API Report] ERROR: SSL Error. Check cert '{SERVER_CERT_PATH_ON_PI}'. Details: {e}")
    except requests.exceptions.Timeout: print(f"  [API Report] ERROR: Timeout connecting to {api_endpoint}")
    except requests.exceptions.ConnectionError: print(f"  [API Report] ERROR: Connection error for {api_endpoint}")
    except requests.exceptions.HTTPError as e: print(f"  [API Report] ERROR: HTTP {e.response.status_code}: {e.response.text}")
    except requests.exceptions.RequestException as e: print(f"  [API Report] ERROR: General failure reporting: {e}")
    return False
...
def main():
    ...
    if os.geteuid() != 0: print("Warning: Root privileges may be required for some functions."); time.sleep(1)
    ...
    try: start_http_server(PROMETHEUS_PORT); print(f"Prometheus metrics server started on port {PROMETHEUS_PORT}")
    except Exception as e: print(f"FATAL: Error starting Prometheus server: {e}\nExiting."); return

    gpio_ok = setup_gpio()
    if not gpio_ok: print("Warning: GPIO setup failed. Button ID reset disabled.")

    print("--- Initializing Device Identifier ---")
    initial_id = load_identifier()
    if not initial_id: initial_id = generate_new_identifier(); save_identifier(initial_id)
    update_prometheus_identifier(initial_id); print("--- Device Identifier Initialized ---")

    print("--- Loading Pi API Key for Registration ---"); load_pi_api_key()
    print("--- Loading Registrar API URL Configuration ---"); load_and_set_registrar_url()
    ...
    try:
        while True:
            ...
            if current_time - last_check_times["ping_scan_ip"] >= PING_SCAN_IP_INTERVAL:
                ...
                if LAN_INTERFACE: update_device_ip(LAN_INTERFACE)
            ...
            if current_time - last_check_times["speedtest"] >= SPEEDTEST_CHECK_INTERVAL:
                if speedtest_process is None:
                    ...
                    speedtest_process = multiprocessing.Process(target=run_speedtest_child, args=(speedtest_queue, st_if), daemon=True)
            ...
            if speedtest_queue is not None:
                try:
                    ...
                    if speedtest_process is not None:
                        ...
                        if speedtest_process.is_alive(): print("  Warning: Speedtest process unclean exit, terminating."); speedtest_process.terminate(); speedtest_process.join(timeout=1)
                    speedtest_process = None
                    if speedtest_queue: speedtest_queue.close(); try: speedtest_queue.join_thread()
                                                         except Exception: pass
                    speedtest_queue = None; print("--- Speedtest Results Processed ---")
                    ...
                    if speedtest_process and not speedtest_process.is_alive():
                         ...
                         if exit_code is None: try: speedtest_process.terminate(); speedtest_process.join(timeout=0.1)
                                               except Exception: pass
                         else: speedtest_process.join(timeout=0)
                    ...
                    if speedtest_process and speedtest_process.is_alive(): print("  Terminating speedtest process due to queue error."); speedtest_process.terminate(); speedtest_process.join(timeout=1)
                    speedtest_process = None; speedtest_queue = None
                    SPEEDTEST_PING.set(-1); DOWNLOAD_SPEED.set(-1); UPLOAD_SPEED.set(-1)
        ...
        if speedtest_process and speedtest_process.is_alive(): print("Terminating active speedtest process..."); speedtest_process.terminate(); speedtest_process.join(timeout=2)
